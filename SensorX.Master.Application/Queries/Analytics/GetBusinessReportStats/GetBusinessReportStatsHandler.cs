...
) : IRequestHandler<GetBusinessReportStatsQuery, Result<BusinessReportStatsResponse>>
{
    public async Task<Result<BusinessReportStatsResponse>> Handle(GetBusinessReportStatsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // CUSTOMER INSIGHTS
            // Customer is Returning if they have an order BEFORE startDate, else New.
            ...
            if (rangeType == "today")
            {
                var blocks = new[] { "08:00", "10:00", "12:00", "14:00", "16:00", "18:00" };
                foreach (var b in blocks)
                {
                    var hour = int.Parse(b.Split(':')[0]);
                    var matchingOrders = orders.Where(o => o.OrderDate.Hour >= hour - 2 && o.OrderDate.Hour < hour);
            ...
            else if (rangeType == "week")
            {
                ...
                var ordersByDay = orders.GroupBy(o => o.OrderDate.DayOfWeek).ToDictionary(g => GetDayNameVi(g.Key), g => g.Sum(o => o.GetGrandTotal().Amount));


                foreach (var day in daysOfWeek)
                {
                    var rev = ordersByDay.TryGetValue(day, out var val) ? val : 0;
            ...
            else if (rangeType == "month")
            {
                ...
                var ordersByDay = orders.GroupBy(o => o.OrderDate.Day).ToDictionary(g => g.Key, g => g.Sum(o => o.GetGrandTotal().Amount));
                for (int d = 1; d <= daysInMonth; d++)
                {
                    var rev = ordersByDay.TryGetValue(d, out var val) ? val : 0;
            ...
            else // year or all
            {
                ...
                var ordersByMonth = orders.GroupBy(o => o.OrderDate.Month).ToDictionary(g => g.Key, g => g.Sum(o => o.GetGrandTotal().Amount));
                for (int m = 1; m <= 12; m++)
                {
                    var rev = ordersByMonth.TryGetValue(m, out var val) ? val : 0;
